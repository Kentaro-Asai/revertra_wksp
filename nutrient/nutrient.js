$(function(){
	...
	$('#explore-select-button, #explore-all-button').on('click', (e)=>{
		...
		if (`explore-all-button` == e.target.id) {
			...
			pn.extractMenu().then(ret_menus => {
				// display nutrients
				$('#output-perfect').html(0 < ret_menus.perfect.length ? displayPerfectMenus(ret_menus.perfect) : ('<p>検索しましたが、1つも発見できませんでした。</p>'+displayPerfectMenus(ret_menus.unperfect)) );
				fitWindowSize();
			});
		}
	});
